...
int main(int argc, char** argv) {
    ...
    if (fscalib.isOpened()) {
        ...
        if (!fscalib["intrinsic"].empty()) {
            READ_MATX(fscalib["intrinsic"], intrinsics);
            fscalib["distortion"] >> distortion;

        } else if (!fscalib["cameraMatrix"].empty()) {
            READ_MATX(fscalib["cameraMatrix"], intrinsics);
            fscalib["dist_coeffs"] >> distortion;
            fscalib["cameraResolution"] >> calib_size;
        }

        if (initialized) {
            device.set(CAP_PROP_FRAME_WIDTH, calib_size.width);
            device.set(CAP_PROP_FRAME_HEIGHT, calib_size.height);
    ...
    while (true) {
        ...
        if (watcher.get_subscribers() > 0) {
            Frame frame{Header("camera", a), make_shared<Tensor>(image)};
